<?php
        ...
        $feedbacks = json_decode(file_get_contents('storage/feedback.json'), true);
        ...
        $encodedId = substr(str_replace(['+', '/', '='], '', base64_encode(pack('H*', md5($uniqueId)))), 0, 6);
        $link = "http://localhost/CourseAssignments/Laravel-Career-Path/Assignment3/anonymous-feedback-app/feedback.php/" . $encodedId; 
        ...
        foreach($users as $key => $value)
        {
            ...
            {
                $users[$key]['link']= $link;
                $feedback= isset($feedbacks[$userEmail]) ? $feedbacks[$userEmail] : '';
...
<header class="bg-white">
    <nav class="flex items-center justify-between p-6 lg:px-8" aria-label="Global">
        ...
        <div class="relative hidden lg:flex lg:flex-1 lg:justify-end">
            <button id="user-menu-button" class="text-sm font-semibold leading-6 text-gray-900 focus:outline-none">
                <?php echo $userName ?>
            </button>
            <div id="user-menu" class="hidden absolute right-0 mt-8 w-48 bg-white border border-gray-200 rounded-lg shadow-lg z-10">
        ...
        <div class="fixed inset-y-0 right-0 z-10 w-full overflow-y-auto bg-white px-6 py-6 sm:max-w-sm sm:ring-1 sm:ring-gray-900/10">
            ...
            <div class="mt-6 flow-root">
                <div class="-my-6 divide-y divide-gray-500/10">
                    <div class="py-6">
                        <button id="mobile-user-menu-button" class="-mx-3 block rounded-lg px-3 py-2.5 text-base font-semibold leading-7 text-gray-900 hover:bg-gray-50">
                            <?php echo $userName ?>
                        </button>
                        <div id="mobile-user-menu" class="hidden mt-3 w-full bg-white border border-gray-200 rounded-lg shadow-lg z-10">
                            <a href="logout.php" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">Logout</a>
                        </div>
                    </div>
                </div>
    ...
    <div class="relative flex min-h-screen overflow-hidden bg-gray-50 py-6 sm:py-12">
        ...
        <div class="relative max-w-7xl mx-auto">
            <div class="flex justify-end">
                <span class="block text-gray-600 font-mono border border-gray-400 rounded-xl px-2 py-1">Your feedback form link: <strong><?php echo $link ?></strong></span>
            </div>
            ...
            <div class="grid grid-cols-1 gap-4 sm:grid-cols-3">
                ...
                <?php 
                    if(!empty($feedback))
                    { 
                    foreach($feedback as $receivedFeedback)
                    {
                ?>

                <div class="relative flex items-center space-x-3 rounded-lg border border-gray-300 bg-white px-6 py-5 shadow-sm focus-within:ring-2 focus-within:ring-indigo-500 focus-within:ring-offset-2 hover:border-gray-400">
                    <div class="focus:outline-none">
                        <p class="text-gray-500"><?php echo $receivedFeedback ?></p>
                    </div>
                </div>
